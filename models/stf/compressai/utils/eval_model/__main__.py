...
def collect_images(rootpath: str) -> List[str]:
    ...
    return [
        ...
        if os.path.splitext(f)[-1].lower() in IMG_EXTENSIONS and f.startswith("brisbane")
    ]
...
def read_image(filepath: str) -> torch.Tensor:
    ...
    if filepath.endswith(".tif") or filepath.endswith(".tiff"):
        img = np.array(tifffile.imread(filepath), dtype=np.float32)
        # fixme: 1/8 band
        # return torch.from_numpy(img).unsqueeze(0) / 65535.0
        return torch.from_numpy(img).permute(2, 0, 1) / 65535.0
    else:
...
@torch.no_grad()
def inference(model, x, filename, recon_path, index, plot=False):
    ...
    psnr_total = psnr(x, out_dec["x_hat"])

    if plot:
        vis_org = x.squeeze().cpu().numpy()[0] * 65535 / 6000
        vis_rec = out_dec["x_hat"].squeeze().cpu().numpy()[0] * 65535 / 6000

        fig, ax = plt.subplots(1, 4, figsize=(20, 5))
        ax[0].imshow(vis_org, cmap="gray")
        ax[0].set_title("Original Image")
        ax[1].imshow(vis_rec, cmap="gray")
        ax[1].set_title(f"Reconstructed Image ({(psnr_total / x.size(1)):.2f}dB, {bpp:.3f}bpp)")
        # fig.suptitle(f"{group_name} - Density {group_density}")
        plt.savefig(f"outputs/plot-{index}.png")
        plt.close(fig)
...
def eval_model(model, filepaths, entropy_estimation=False, half=False, recon_path="reconstruction"):
    ...
    with tqdm(total=len(filepaths), desc="Evaluating") as pbar:
        for index, f in enumerate(filepaths):
            _filename = f.split("/")[-1]
            ...
            if not entropy_estimation:
                if half:
                    model = model.half()
                    x = x.half()
                rv = inference(model, x, _filename, recon_path, index, plot)
                plot = False
            else:
            ...
            bpp_total += rv["bpp"]
            image_count += 1
            channel_count += x.size(0)

            pbar.set_postfix({"psnr": psnr_total / channel_count, "bpp": bpp_total / image_count})
            # pbar.set_postfix({k: v / (pbar.n or 1) for k, v in metrics.items()})
