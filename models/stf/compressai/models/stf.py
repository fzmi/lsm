...
try:
    ...
    from models.stf.compressai.layers import conv3x3, subpel_conv3x3
...
class SymmetricalTransFormer(CompressionModel):
    ...
    def compress_and_reconstruct_batch(self, x, max_workers: int = 8):
        ...
        Returns:
            ...
            decode_time: float, wall-clock seconds for NN decoder (Swin decoder) only.
        """
        from concurrent.futures import ThreadPoolExecutor

        N = x.shape[0]

        # --- Swin encoder (batched on GPU) ---
        t_enc_start = time.time()
        x_enc = self.patch_embed(x)
        Wh, Ww = x_enc.size(2), x_enc.size(3)
        x_enc = x_enc.flatten(2).transpose(1, 2)
        for layer in self.layers:
            x_enc, Wh, Ww = layer(x_enc, Wh, Ww)
        C = self.embed_dim * 8
        y = x_enc.view(-1, Wh, Ww, C).permute(0, 3, 1, 2).contiguous()
        y_shape = y.shape[2:]
        ...
        # --- Swin decoder (batched, no decompress() needed) ---
        y_hat = torch.cat(y_hat_slices, dim=1)
        torch.cuda.synchronize()
        t_dec_start = time.time()
        y_hat_seq = y_hat.permute(0, 2, 3, 1).contiguous().view(-1, Wh * Ww, C)
        for layer in self.syn_layers:
            y_hat_seq, Wh, Ww = layer(y_hat_seq, Wh, Ww)
        x_hat_cpu = (
            self.end_conv(
                y_hat_seq.view(-1, Wh, Ww, self.embed_dim)
                .permute(0, 3, 1, 2).contiguous()
            ).clamp_(0, 1)[:, 0].detach().cpu()
        )  # [N, H, W]
        t_dec_end = time.time()
