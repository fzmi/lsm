...
from models.mlic.utils.func import update_registered_buffers, get_scale_table
from models.mlic.utils.ckbd import *
from models.mlic.modules.transform import *
...
class MLICPlusPlus(CompressionModel):
    ...
    def compress_and_reconstruct_batch(self, x, max_workers: int = 8):
        """Batched encode + reconstruct without calling decompress().

        Args:
            x: [N, 1, H, W] tensor on device (values in [0, 1]).
        Returns:
            bits_list: list of N ints — total bits for each element.
            x_hat_cpu: [N, H, W] float32 on CPU, values in [0, 1].
            encode_time: float, wall-clock seconds for NN encoder + entropy coding.
            decode_time: float, wall-clock seconds for NN decoder (g_s) only.
        """
        ...
        # Per-element symbol/index accumulation
        all_syms = [[] for _ in range(N)]
        all_idxs = [[] for _ in range(N)]

        def _collect_anchor(anchor, scales_anchor, means_anchor):
            """Batched compress_anchor: quantizes + collects per-element symbols."""
            anchor_sq = ckbd_anchor_sequeeze(anchor)
            scales_sq = ckbd_anchor_sequeeze(scales_anchor)
            means_sq = ckbd_anchor_sequeeze(means_anchor)
            indexes = self.gaussian_conditional.build_indexes(scales_sq)
            anchor_hat = self.gaussian_conditional.quantize(anchor_sq, "symbols", means_sq)
            for b in range(N):
                all_syms[b].extend(anchor_hat[b].reshape(-1).tolist())
                all_idxs[b].extend(indexes[b].reshape(-1).tolist())
            return ckbd_anchor_unsequeeze(anchor_hat + means_sq)

        def _collect_nonanchor(nonanchor, scales_nonanchor, means_nonanchor):
            """Batched compress_nonanchor: quantizes + collects per-element symbols."""
            nonanch_sq = ckbd_nonanchor_sequeeze(nonanchor)
            scales_sq = ckbd_nonanchor_sequeeze(scales_nonanchor)
            means_sq = ckbd_nonanchor_sequeeze(means_nonanchor)
            indexes = self.gaussian_conditional.build_indexes(scales_sq)
            nonanch_hat = self.gaussian_conditional.quantize(nonanch_sq, "symbols", means_sq)
            for b in range(N):
                all_syms[b].extend(nonanch_hat[b].reshape(-1).tolist())
                all_idxs[b].extend(indexes[b].reshape(-1).tolist())
            return ckbd_nonanchor_unsequeeze(nonanch_hat + means_sq)
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                scales_anchor = ckbd_anchor(scales_anchor)
                means_anchor = ckbd_anchor(means_anchor)
                slice_anchor = _collect_anchor(slice_anchor, scales_anchor, means_anchor)
                lrp_anchor = self.lrp_anchor[idx](torch.cat(([hyper_means] + y_hat_slices + [slice_anchor]), dim=1))
                slice_anchor = slice_anchor + ckbd_anchor(lrp_anchor)
                local_ctx = self.local_context[idx](slice_anchor)
                params_nonanchor = self.entropy_parameters_nonanchor[idx](torch.cat([local_ctx, hyper_params], dim=1))
                ...
                scales_nonanchor = ckbd_nonanchor(scales_nonanchor)
                means_nonanchor = ckbd_nonanchor(means_nonanchor)
                slice_nonanchor = _collect_nonanchor(slice_nonanchor, scales_nonanchor, means_nonanchor)
                lrp_nonanchor = self.lrp_nonanchor[idx](torch.cat(([hyper_means] + y_hat_slices + [slice_nonanchor + slice_anchor]), dim=1))
                slice_nonanchor = slice_nonanchor + ckbd_nonanchor(lrp_nonanchor)
                y_hat_slices.append(slice_nonanchor + slice_anchor)
            else:
        ...
        # Batched NN decoder — no decompress() needed
        y_hat = torch.cat(y_hat_slices, dim=1)
        torch.cuda.synchronize()
        t_dec_start = time.time()
        x_hat_cpu = self.g_s(y_hat).clamp_(0, 1)[:, 0].detach().cpu()  # [N, H, W]
        t_dec_end = time.time()
        decode_time = t_dec_end - t_dec_start

        # Per-element entropy encoding in parallel threads (C extension releases GIL)
        def _encode_one(b):
            enc = BufferedRansEncoder()
            enc.encode_with_indexes(all_syms[b], all_idxs[b], cdf, cdf_lengths, offsets)
            y_str = enc.flush()
            return (len(y_str) + len(z_strings[b])) * 8

        workers = min(max_workers, N)
        if workers <= 1:
            bits_list = [_encode_one(b) for b in range(N)]
        else:
            with ThreadPoolExecutor(max_workers=workers) as pool:
                bits_list = list(pool.map(_encode_one, range(N)))

        t_enc_end = time.time()
        encode_time = (t_dec_start - t_enc_start) + (t_enc_end - t_dec_end)
        return bits_list, x_hat_cpu, encode_time, decode_time
