...
try:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "compressai is required for STF training. Install via `pip install compressai`."
        ) from exc

from models.stf.data import STFPatchDataset
...
def _resolve_save_path(template: Optional[str], lambda_value: float, arch: Optional[str], fold: Optional[int]) -> Optional[Path]:
    if template is None:
        return None
    return Path(_format_template(template, lambda_value, fold=fold, arch=arch))


def _resolve_checkpoint_path(cfg: TrainConfig, lambda_value: float, arch: Optional[str], fold: Optional[int]) -> Optional[Path]:
    if cfg.checkpoint_template:
        return Path(_format_template(cfg.checkpoint_template, lambda_value, fold=fold, arch=arch))
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output: Mapping[str, torch.Tensor], target: torch.Tensor) -> Dict[str, torch.Tensor]:
        n, _, h, w = target.size()
        num_pixels = n * h * w
        # Cast to float32 to avoid float16 precision issues under AMP
        bpp = sum(
            (torch.log(likelihoods.float().clamp_min(EPS)).sum() / (-math.log(2.0) * num_pixels))
            for likelihoods in output["likelihoods"].values()
        )
        mse = self.mse(output["x_hat"].float(), target.float())
        loss = self.lmbda * 255.0 ** 2 * mse + bpp
        return {"loss": loss, "bpp_loss": bpp, "mse_loss": mse}
...
def configure_optimizers(model: nn.Module, cfg: TrainConfig):
    params = {name for name, p in model.named_parameters() if p.requires_grad and not name.endswith(".quantiles")}
    aux_params = {name for name, p in model.named_parameters() if p.requires_grad and name.endswith(".quantiles")}

    params_dict = dict(model.named_parameters())
    if params & aux_params:
        raise RuntimeError("Overlap detected between main and auxiliary parameters")

    optimizer = optim.Adam((params_dict[n] for n in sorted(params)), lr=cfg.learning_rate)
    aux_optimizer = optim.Adam((params_dict[n] for n in sorted(aux_params)), lr=cfg.aux_learning_rate)
...
def _init_wandb(cfg: TrainConfig):
    ...
    run = wandb.init(project="lsm-stf", config=config)
    return run
...
def train_one_epoch(
    ...
    for batch_idx, data in enumerate(progress, start=1):
        ...
        # bf16 autocast: ~2x throughput on H100 for the SwinTransformer backbone.
        # GradScaler not needed for bf16 (full dynamic range). RateDistortionLoss
        # casts likelihoods/x_hat to float() internally, so the loss math runs
        # in fp32 even under autocast.
        ...
        if not torch.isfinite(loss_main).item():
            progress.write("Non-finite main loss encountered; skipping batch")
            continue
        bpp_loss = losses.get("bpp_loss")
        mse_loss = losses.get("mse_loss")
        if bpp_loss is not None and not torch.isfinite(bpp_loss).item():
            progress.write("Non-finite bpp loss encountered; skipping batch")
            continue
        if mse_loss is not None and not torch.isfinite(mse_loss).item():
            progress.write("Non-finite mse loss encountered; skipping batch")
            continue
        ...
        if not torch.isfinite(aux_loss).item():
            progress.write("Non-finite auxiliary loss encountered; skipping batch")
            continue
        aux_loss.backward()
        aux_optimizer.step()
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SymmetricalTransFormer on LSM patches")
    ...
    parser.add_argument("--test-batch-size", type=int, default=16, help="Validation batch size")
    parser.add_argument("--epochs", type=int, default=40, help="Number of epochs")
    parser.add_argument("--learning-rate", type=float, default=5e-5, help="Main optimizer learning rate")
    parser.add_argument("--aux-learning-rate", type=float, default=5e-4, help="Auxiliary optimizer learning rate")
    parser.add_argument("--lambda", dest="lmbda", type=float, default=1e-2, help="Rate-distortion lambda")
    ...
    parser.add_argument("--save-path", type=str, default="outputs/stf/{arch}/lambda{lambda_tag}.pth", help="Checkpoint path or template (supports {lambda_value} / {lambda_tag} / {fold} / {arch})")
    parser.add_argument("--checkpoint", type=str, help="Checkpoint path or template for resuming (supports {lambda_value} / {lambda_tag} / {fold} / {arch})")
    parser.add_argument("--no-save", action="store_true", help="Disable checkpoint saving")
    parser.add_argument("--wandb", action="store_true", help="Enable Weights & Biases logging")
    parser.add_argument("--model", choices=["stf", "cnn"], default="stf", help="Model architecture to train")
...
def build_dataloader(regions: List[str], cfg: TrainConfig, shuffle: bool, subset_training: bool = True) -> DataLoader:
    dataset = STFPatchDataset(cfg.dataset_path, regions, (cfg.patch_size, cfg.patch_size))
...
def train_for_lambda(run_cfg: TrainConfig, train_loader: DataLoader, val_loader: DataLoader, model_name: str) -> None:
    prefix = f"[Fold {run_cfg.fold}] " if run_cfg.fold is not None else ""
    print(f"{prefix}Training {model_name.upper()} model with lambda={run_cfg.lmbda}")
    model_cls = compressai_models[model_name]
    model = model_cls().to(run_cfg.device)
    ...
    criterion = RateDistortionLoss(run_cfg.lmbda)
    ...
    for epoch in range(start_epoch, run_cfg.epochs):
        ...
        print(
            f"{prefix}Epoch {epoch}: train_psnr={avg_train_psnr:.2f}, val_loss={metrics['loss']:.6f}, val_bpp={metrics['bpp']:.4f}, val_mse={metrics['mse']:.6f}, val_aux={metrics['aux']:.6f}"
        )
    ...
    print(f"{prefix}Finished training {model_name.upper()} lambda={run_cfg.lmbda} (best validation loss {best_loss:.6f})")
    ...
    if args.cross_validate:
        for fold_idx, group in enumerate(CROSSVAL_GROUPS, start=1):
            ...
            for lambda_value in lambda_values:
                save_path = _resolve_save_path(base_cfg.save_template, lambda_value, arch, fold_idx)
                ...
                checkpoint_path = _resolve_checkpoint_path(base_cfg, lambda_value, arch, fold_idx)
                ...
                train_loader = build_dataloader(run_cfg.train_regions, run_cfg, shuffle=True, subset_training=True)
                val_loader = build_dataloader(run_cfg.val_regions, run_cfg, shuffle=False, subset_training=False)
                train_for_lambda(run_cfg, train_loader, val_loader, arch)
        return

    train_loader = build_dataloader(base_cfg.train_regions, base_cfg, shuffle=True, subset_training=True)
    val_loader = build_dataloader(base_cfg.val_regions, base_cfg, shuffle=False, subset_training=False)

    for lambda_value in lambda_values:
        save_path = _resolve_save_path(base_cfg.save_template, lambda_value, arch, None)
        checkpoint_path = _resolve_checkpoint_path(base_cfg, lambda_value, arch, None)
